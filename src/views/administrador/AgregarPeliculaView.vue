    ...
    <v-container>
        <v-card class="pa-3">
            <v-form @submit.prevent="addPelicula">
                <v-autocomplete v-model="search" :items="results" item-text="title" label="Buscar Película"
                ...
                <v-btn type="submit" color="primary">Agregar Película</v-btn>
            </v-form>
        </v-card>
    </v-container>
</template>
...
export default {
    ...
    methods: {
        async searchMovie() {
            try {
                const response = await this.$store.dispatch('BuscarPelicula', { query: this.search });
